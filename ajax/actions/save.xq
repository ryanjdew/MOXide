...
declare variable $static-check as xs:boolean := 
		try {
			xdmp:eval($module,(),<options xmlns="xdmp:eval"><static-check>true</static-check><root>{$app-root}</root></options>),
            true()
		} catch($e) {
            error(
                QName("http://maxdewpoint.blogspot.com/moxide","STATIC_CHECK"),
                concat("This module failed to pass a static check: ",$e/*:name)
            )
		};
try {
